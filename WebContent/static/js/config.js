 ...
 * @time 创建时间：2016年12月15日 下午5:43:15
 * @description
 */

function changeHtmlSpecialChar(v) {
	// g,表示全部替换。
...
var s1 = [ "homeTown_province", "homeTown_city", "homeTown_country" ];
var s2 = [ "nowLoc_province", "nowLoc_city", "nowLoc_country" ];
var opt0 = [ "省份", "地级市", "市、县级市、县" ];
function setup() {
	for (i = 0; i < s1.length - 1; i++)
		document.getElementById(s1[i]).onchange = new Function("changeS1("
				+ (i + 1) + ")");
	changeS1(0);
	for (i = 0; i < s2.length - 1; i++)
		document.getElementById(s2[i]).onchange = new Function("changeS2("
				+ (i + 1) + ")");
	changeS2(0);
}
function Dsy() {
...
function getRightFrame() {
	var frames = window.parent.window.document.getElementById("right");
	return frames;
}
 ...
 * 返回首页blog
 */
function returnBlog(btnId,menuId){
    $("."+btnId).click(function(){
	/*var headFrame = getHeadFrame();
	var coding = $(headFrame).contents().find("#coding");*///jquery获取iframe中的元素之前需要加上.comtents()
...
dsy.add("0_30_0", [ "城口县", "大足县", "垫江县", "丰都县", "奉节县", "合川市", "江津市", "开县",
		...
		"璧山县", "綦江县" ]);
/* 提示框 */
function showAlert($body, msg) {
	var html = '<div class="modal" id="alertModal" role="dialog"><div class="modal-dialog alert-dialog"><div class="modal-content">'
			+ '<div class="modal-header"><h4 class="modal-title center">温馨提示</h4></div>'
			+ '<div class="modal-body"><p class="center">'
			+ msg
			+ '</p></div>'
			+ '<div class="modal-footer"><button type="button" class="btn btn-sm btn-info btn-yes" data-dismiss="modal">确定</button></div>'
			+ '</div></div></div>';
	$body.append(html);
	$("#alertModal").modal({
		keyboard : false,
	...
	$("#alertModal").find(".modal-footer").css("height","40px");
}