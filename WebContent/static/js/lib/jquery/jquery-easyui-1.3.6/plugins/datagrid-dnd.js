		...
		enableDnd: function(jq, index){
			return jq.each(function(){
				...
				trs.draggable({
					...
					onDrag: function(e) {
						...
						if (d>3){	// when drag a little distance, show the proxy object
							$(this).draggable('proxy') && $(this).draggable('proxy').show();
					...
					onStopDrag:function( e ){
						...
						opts.onStopDrag.call(target,e, state.draggingRow);*/
				...
				function allowDrop(source, allowed){
					var icon = $(source).draggable('proxy') && $(source).draggable('proxy').find('span.tree-dnd-icon');
                ...
                function getDraggingRow(tr){
                    //$(tr).attr('datagrid-row-index')
                    if (!$(tr).hasClass('datagrid-row')){return null}
                    var target = $(tr).closest('div.datagrid-view .datagrid-view2 .datagrid-body').children('table')[0];
                    var opts = $("#unAuthorizedGrid").datagrid('options');
                    var opt2s = $(target).datagrid('options');
                    if (!opts.singleSelect){
                        //if (opts.dragSelection){
                       // var rows = $(target).datagrid('getSelections');
                        var rows=$("#unAuthorizedGrid").datagrid('getSelections');
